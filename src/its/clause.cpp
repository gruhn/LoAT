...
const std::optional<Var> varAt(const Var &var, const Subs &subs) {
    if (!subs.contains(var)) {
        ...
        // TODO: there's probably a simpler way to restore a BoolVar from a
        // BoolExpr?
 ...
 *
 */
const std::optional<Subs> computeUnifier(const FunApp &pred1,
                                         const FunApp &pred2) {
...
template <typename T>
const std::vector<T> concat(const std::vector<T> vec1,
 ...
 *
 */
const std::optional<Clause> removeFromLHS(const Clause &chc,
                                          const FunApp &pred) {
  ...
  while (pred_pos < chc.lhs.end()) {
    if (&(*pred_pos) ==
        &pred) { // TODO: does this make sense? I want to compare by reference
      break;
    }
    pred_pos++;
  }
 ...
 *
 */
const std::optional<Clause> resolutionWith(const Clause &fst, const Clause &snd,
                                           const FunApp &pred) {
  std::cout << "test1" << std::endl;
  const auto snd_without_pred_optional = removeFromLHS(snd, pred);
  std::cout << "test2" << std::endl;
  assert(snd_without_pred_optional.has_value() &&
         "Given `pred` is not on the LHS of `snd`");
  ...
  return Clause(concat(fst_renamed.lhs, snd_without_pred.lhs), // resolvent LHS
